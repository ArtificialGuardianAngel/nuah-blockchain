...
import { MsgCreateRequest } from "./types/nuah/requests/tx";

import { Params as typeParams} from "./types"
import { Requests as typeRequests} from "./types"

export { MsgCreateRequest };
...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgCreateRequest({ value }: msgCreateRequestParams): EncodeObject {
			try {
				return { typeUrl: "/nuah.requests.MsgCreateRequest", value: MsgCreateRequest.fromPartial( value ) }  
			} catch (e: any) {
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			NuahRequests: new SDKModule(test)
		},
		registry: msgTypes
  }
}
