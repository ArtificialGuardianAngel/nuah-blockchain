...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		async sendMsgCheckName({ value, fee, memo }: sendMsgCheckNameParams): Promise<DeliverTxResponse> {
			if (!signer) {
					throw new Error('TxClient:sendMsgCheckName: Unable to sign Tx. Signer is not present.')
			}
			try {			
				const { address } = (await signer.getAccounts())[0]; 
				const signingClient = await SigningStargateClient.connectWithSigner(addr,signer,{registry, prefix});
				let msg = this.msgCheckName({ value: MsgCheckName.fromPartial(value) })
				return await signingClient.signAndBroadcast(address, [msg], fee ? fee : defaultFee, memo)
			} catch (e: any) {
				throw new Error('TxClient:sendMsgCheckName: Could not broadcast Tx: '+ e.message)
			}
		},
		...
		msgTransferName({ value }: msgTransferNameParams): EncodeObject {
			try {
				return { typeUrl: "/nuah.nameservice.MsgTransferName", value: MsgTransferName.fromPartial( value ) }  
			} catch (e: any) {
				throw new Error('TxClient:MsgTransferName: Could not create message: ' + e.message)
			}
		},
...
const Module = (test: IgniteClient) => {
	return {
		module: {
			NuahNameservice: new SDKModule(test)
		},
		registry: msgTypes
  }
}
