...
export const RequestBookItem = {
  ...
  fromJSON(object: any): RequestBookItem {
    return {
      ...
      requests: Array.isArray(object?.requests)
        ? object.requests.map((e: any) => Request.fromJSON(e))
        : [],
    };
  },
  ...
  toJSON(message: RequestBookItem): unknown {
    const obj: any = {};
    message.idCount !== undefined &&
      (obj.idCount = Math.round(message.idCount));
    if (message.requests) {
  ...
  fromPartial<I extends Exact<DeepPartial<RequestBookItem>, I>>(
    object: I
  ): RequestBookItem {
    const message = createBaseRequestBookItem();
    message.idCount = object.idCount ?? 0;
    message.requests =
      object.requests?.map((e) => Request.fromPartial(e)) || [];
    return message;
  },
};
